...
import org.saturn.app.command.impl.user.WhiskeySayUserCommandImpl;
...
class WhiskeySayUserCommandImplTest {
  ...
  void executeTest() {
    // Setup
    doReturn("*whiskeysay test message").when(message).getText();
    doReturn("*").when(engine).getPrefix();
    doReturn("testAuthor").when(message).getNick();
    
    WhiskeySayUserCommandImpl cmd = new WhiskeySayUserCommandImpl(engine, message, List.of());
