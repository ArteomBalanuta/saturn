  ...
  public Optional<Status> execute() {
    ...
    if (getArguments().size() != 2 || trip.isEmpty()) {
      engine.outService.enqueueMessageForSending(
          author,
          "\\n Set your trip first. Example: " + engine.prefix + "grant 8Wotmg ADMIN",
          isWhisper());
    ...
    if (trips.isEmpty()) {
      ...
      engine.outService.enqueueMessageForSending(
          author,
          "\\n Granted new Role: " + newRole.name() + " to trip: " + targetTrip,
          isWhisper());
    } else {
    ...
    log.info(
        "Executed [grant] command by user: {}, trip: {}, new Role: {} for trip: {}",
        author,
        trip.get(),
        newRole.name(),
        targetTrip);
